...
echo "1) اعمال محدودیت"
echo "2) لغو محدودیت"
read -p "لطفا گزینه مورد نظر خود را وارد کنید: " menu_choice
...
if [ "$menu_choice" == "1" ]; then
    ...
    if [ -z "$username" ]; then
        echo "لطفا نام کاربری را وارد کنید."
        exit 1
    fi

    if ! id "$username" &>/dev/null; then
        echo "کاربر $username وجود نдارد."
        exit 1
    fi

    sed -i "/^$username hard maxlogins/d" /etc/security/limits.conf

    echo "محдودیت برای کاربر $username با موفقیت لغو شد."
